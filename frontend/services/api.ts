...
apiClient.interceptors.request.use((config) => {
  ...
  if (config.data instanceof FormData) {
    ...
    console.log('Request interceptor: Removed Content-Type for FormData, kept Authorization:', {
...
apiClient.interceptors.response.use(
  ...
  (error) => {
    ...
    if (error.response?.status === 401 && typeof window !== 'undefined') {
      ...
      if (currentPath === '/profile' || requestUrl.includes('/profile') || isRecentLogin || isAuthRequest) {
        // Don't clear token for profile page, recent logins, or auth requests
        console.log('401 on profile/recent login/auth request - not clearing token');
